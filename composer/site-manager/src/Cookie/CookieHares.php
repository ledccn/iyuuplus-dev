 ...
 * - 凭cookie解析HTML列表页
 */
class CookieHares extends BaseCookie
{
    ...
    public function process(string $url): array
    {
        ...
        $table = Selector::select($html, "//table[contains(@class,'torrents')]");
        if (empty($table)) {
        ...
        foreach ($list as $v) {
            ...
            if (preg_match('/details.php\?id=(?<id>\d+)/i', $v, $matches_id)) {
                ...
                if (preg_match('/(?<download>download.php\?.*?)[\'|\"]/i', $v, $matches_download)) {
                    ...
                    // 获取主标题
                    $arr['h1'] = Helper::makeCrawler($v)->filterXPath('//div[contains(@class,"layui-torrents-title")]//a')->attr('title', '');
                    // 副标题
                    $arr['title'] = $this->normalizeWhitespace(Helper::makeCrawler($v)->filterXPath('//p')->text(''));
